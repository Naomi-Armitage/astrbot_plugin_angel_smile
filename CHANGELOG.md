...
## 1.0.4 - 2026-03-24
- 合并官方 `1.0.3` 的渲染修复：仅匹配当前可用的表情名，避免把普通 `:xxx:` 文本误判成表情标签。
- 保留并整理本地图片真实格式修复：保存偷取图片时按实际格式决定后缀，避免误存成错误扩展名。
- 补齐渲染回归测试与测试桩，覆盖时间字符串场景，并保持消息组件假实现可用。
...
- 入库流程按图片真实格式决定保存后缀，避免上游统一改成 `.jpg` 时导致错误扩展名落盘。
